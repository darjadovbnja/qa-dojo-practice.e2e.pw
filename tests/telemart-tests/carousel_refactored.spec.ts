...
test('active banner navigation', async({page}) => {
   ...
   await verifyNumberOfBanners(carouselBanner);
    ...
    const urlOfActiveBanner = await getActiveBannerUrl(carouselActiveBanner);
    console.log(urlOfActiveBanner);
        
    await carouselActiveBanner.click()
    
    await expect(removeUaFromUrl(page.url())).toEqual(urlOfActiveBanner);
});